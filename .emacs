...
;;  desativar uma função de autocompletar específica no minibuffer
...
;;; compilation corretor de cor no compilation mode
...
;;; autoPair direto do .emacs.d/lisp/autopair.el
(require 'autopair)
(autopair-global-mode)


;;  ;;; autoPair direto do .emacs.d/lisp/key-chord.el
(require 'key-chord)
      (key-chord-mode 1)
(key-chord-define-global "<<" 'smart-shift-left)

;;atalhos de teclado disponíveis prefixo (como C-x)
...
;;; autoPair direto do .emacs.d/lisp/simpc-mode.el 
(require 'simpc-mode)
...
;;; treemacs Treemacs exiba informações úteis fornecidas pelo servidor de linguagem (LSP)
...
;;;tree-sitter-langs ~/.emacs.d/tree-sitter/
(require 'treesit)
(treesit-available-p)
...
(defun lsp-booster--advice (orig-fn &rest args)
  ...
  (let* ((server-id (or (car (plist-get (plist-get args :plist) :server-id)
    ...
    (when (and (not (file-remote-p default-directory))
					(memq server-id '(pylsp rust-analyzer pyright ts-ls jdtls gopls)))
